...
<li id="menu-item-264" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-264"><a href="pages.php?title=<?php echo $title; ?>">Pages</a></li>
	...
	<div id="header-container">
		...
		<div class="row">		
	  	...
	  	<a href="http://ind.php" title="Noun Based Social Search" rel="home">
...
<h1 class="animated BeanFadeDown"><?php echo $title; ?> - The Social Pages						</h1>
...
<div class="twelve columns">	
	...
	<?php
				...
				$type = 'post';
				$types = 'page';
				$retrive = $facebook->api('/search?q='.$query.'&type='.$type.'&limit=20');

				$string= json_encode($retrive );
				$json_a = json_decode($string, true);
				$json_o = json_decode($string);
				
				$retrives = $facebook->api('/search?q='.$query.'&type='.$types.'&limit=25');

				$strings= json_encode($retrives );
				$json_as = json_decode($strings, true);
				$json_os = json_decode($strings);
	...
	<div class="related-posts bean-category">
					...
					<ul>
						...
						<?php foreach($json_os->data as $p)
							{
								$text = $p->message;
								$username=$p->name;
								$id=$p->id;
								echo "<li class='cat'><img src='https://graph.facebook.com/".$id."/picture' width='50' height='50'  />
									  <a href='"."http://www.facebook.com/".$id."' target='new'>$username</a></li>";
							}
						?>											
						<!-- END li.cat -->																							
					</ul>
